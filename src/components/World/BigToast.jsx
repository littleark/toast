...
const BigToast = (props) => {
  ...
  const breadSize = bigToastScale(toast.a)
  ...
  return <g id='bigToast' transform={`translate(${bigToastX},${bigToastY})`}>
    <g transform={`translate(${-breadSizeHalf},${-bigToastScale(toast.a)}) scale(${SIZE_FACTOR * (toast.a / sizeCM)})`}>
    ...
    <g className='size' transform={`translate(${-breadSizeHalf},0)`}>
      <path className='bread-size' d={`m0,${-breadSize} l0,-5 l${breadSize},0 l0,5`} />
      <rect />
      <text className='bread-size' x={breadSizeHalf} y={-(breadSize + 10)}>{d3Format(',.3f')(toast.a) * 100 + 'cm'}</text>
      <text className='hangout' x={(breadSize - hangout + (breadSize - hangout + hangout)) / 2} y={25}>{(d3Format(',.3f')(toast.a / 2 + toast.r) * 100) + 'cm'}</text>
      <path className='hangout' d={`m${breadSize - hangout},5 l0,5 l${hangout},0 l0,-5`} />
    </g>
  </g>
}
